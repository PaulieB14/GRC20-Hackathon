// add-property-addresses-to-triples.cjs
const fs = require('fs');
 ...
 */
function addPropertyAddressesToTriples() {
  try {
    // Read the deeds-triples-updated.json file
    const deedsTriples = JSON.parse(fs.readFileSync('data/deeds-triples-updated.json', 'utf-8'));
    ...
    fs.writeFileSync('data/deeds-triples-updated.json', JSON.stringify(deedsTriples, null, 2));
