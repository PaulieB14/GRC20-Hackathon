 ...
 */
async function updateGRC20Triples() {
    try {
        ...
        const instrumentNumberToEntityId = new Map();
        ...
        const oldToNewEntityId = new Map();
        // Match old entity IDs to new entity IDs based on instrument number
        for (const entity of deedsTriples) {
            const instrumentNumberTriple = entity.triples.find((triple) => triple.attributeId === 'LuBWqZAu6pz54eiJS5mLv8');
            if (instrumentNumberTriple) {
                const instrumentNumber = instrumentNumberTriple.value.value;
        ...
        const updatedDeedsTriples = deedsTriples.map((entity) => {
