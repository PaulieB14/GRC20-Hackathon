...
import { Graph, Relation, Triple } from "@graphprotocol/grc-20";
...
async function implementRelations() {
    try {
        ...
        const { buyerRelationTypeId, sellerRelationTypeId, documentTypeRelationTypeId, personTypeId, documentTypeTypeId, nameAttributeId, addressAttributeId, deedNumberAttributeId } = await createRelationTypes(ops);
        ...
        for (const info of entityInfo.values()) {
            if (info.buyer)
                uniquePersons.add(info.buyer);
            if (info.seller)
                uniquePersons.add(info.seller);
        }
        // Create person entities
 ...
 */
async function createRelationTypes(ops) {
    // Create person type
 ...
 */
async function publishUpdate(ops) {
    try {
