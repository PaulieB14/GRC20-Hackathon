...
import { Graph, type Op, Ipfs, Triple } from '@graphprotocol/grc-20';
 ...
 */
async function findDeedEntityId(instrumentNumber: string): Promise<string | null> {
  try {
 ...
 */
export async function createDeedDocumentOps(instrumentNumber: string, documentUrl: string): Promise<Op[]> {
  try {
    const ops: Op[] = [];
 ...
 */
async function publishDeedUpdate(ops: Op[], instrumentNumber: string): Promise<void> {
  try {
 ...
 */
export async function uploadDeedPdf(
  instrumentNumber: string, 
  filePath: string, 
