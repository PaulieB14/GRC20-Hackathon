...
/**
 * Publishes updates to existing GRC20 deed entities
 */
export async function publishGRC20DeedUpdates() {
  try {
    ...
    const editName = 'Update GRC20 Property Deeds';

    // Import the updateGRC20Deeds module
    console.log('\n[Transform] Getting ops from updateGRC20Deeds...');
    const updateGRC20DeedsModule = await import('./update-grc20-deeds.js');
    const ops = await updateGRC20DeedsModule.updateGRC20Deeds();
    
    if (!ops) {
      throw new Error('Failed to generate update operations');
    }
...
if (import.meta.url === new URL(import.meta.url).href) {
  console.log('[Startup] Starting GRC20 deed updates publication...');
  
  publishGRC20DeedUpdates().catch(error => {
    console.error('\n❌ [Error]:', error);
    process.exit(1);
  });
}
