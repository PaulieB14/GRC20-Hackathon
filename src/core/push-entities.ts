...
import { Graph, Triple, type Op } from '@graphprotocol/grc-20';
...
/**
 * Pushes entities to the knowledge graph
 */
async function pushEntities(): Promise<void> {
  try {
    ...
    for (const record of records) {
      const { id: entityId, ops: entityOps } = Graph.createEntity({
        ...
        types: [deedTypeId],
        properties: {
          // Properties will be added in a separate step
        },
      });
      ops.push(...entityOps);
      
      console.log(`Created entity ${entityId} for instrument number ${record.InstrumentNumber}`);
    }
    ...
    // Publish the edit
    const editName = 'Push GRC20 Deed Entities';
    ...
    console.error('Failed to push entities:', error);
    throw error;
  }
}
