 ...
 */
async function pushEntitiesWithAddresses(): Promise<void> {
  try {
    ...
    for (const record of records) {
      // Create a more descriptive name instead of using the instrument number
      const entityName = `Deed: ${record.DirectName} to ${record.IndirectName}`;
      
      // Create the entity with a descriptive name
