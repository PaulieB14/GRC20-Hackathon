 ...
 */
function updateDeedsTriples(filePath: string, entityMap: Map<string, string>): string[] {
  try {
    ...
    for (const entity of triples) {
      ...
      const instrumentNumberTriple = entity.triples.find(t => 
 ...
 */
function updatePermitsTriples(filePath: string, entityMap: Map<string, string>): boolean {
  try {
    console.log(`Updating ${filePath} with ${entityMap.size} entity IDs...`);
    ...
    for (const entity of triples) {
      ...
      const recordNumberTriple = entity.triples.find(t => 
        t.value && t.value.type === 'TEXT' && 
 ...
 * @returns True if the file was updated, false otherwise
 */
function updateWorkingDeedIds(filePath: string, entityIds: string[]): boolean {
  try {
